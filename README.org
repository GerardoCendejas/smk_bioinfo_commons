...
Input and output files are always defined with a name (~txt~ and ~csv~ in this example), see specific rules in [[Modules][modules]] for the specific input and output files that are defined for each rule, these names are descriptors of the file type, and are used to define the input and output files in the rule.
...
** [[file:workflow/rules/ahmm.smk][ahmm]]

Analyzes the results of a run from [[https://github.com/russcd/Ancestry_HMM][ancestry hmm]].

** [[file:workflow/rules/bcftools.smk][bcftools]]

General rules for working with ~.bcf~ and ~.vcf~ files.

** [[file:workflow/rules/admixture.smk][admixture]]

General rules for running admixture on a ~.bcf~.

** [[file:workflow/rules/demesdraw.smk][demesdraw]]

Rules for drawing demographic models in demesformat.

** [[file:workflow/rules/dsuite.smk][dsuite]]
...
** [[file:workflow/rules/genomics_general.smk][genomics_general]]

Rules for running analysis from [[https://github.com/simonhmartin/genomics_general.git][genomics general]] package. includes functions to get maximum likelihood phylogenetic trees from ~.vcf~ files in genomic windows by using phyml.

** [[file:workflow/rules/liftoff.smk][liftoff]]

Rules for running liftoff to lift over annotations from one genome to another.

** [[file:workflow/rules/phlash.smk][phlash]]
...
To distinguish between the [[log][log]] files explicitely determined by the output of a rule from the log files of snakemake... the ~rlog~ flag in a rule indicates that this log file is the log file from a previously run rule, so it's easier to remember this.
...
A type of [[txt][txt]] file, which represents one sample per line, this is the sample ids that are used in different formats such as [[bcf][bcf]].
...
A [[pop_map][pop_map]] file, but expanded to have the two haplotypes of each sample, where A and B are two haplotypes of the same individual.
...
A format for genotype info, needed for genomics_general pipelines.
